      ...
      <div className="relative w-[90vw] max-w-4xl h-[85vh] bg-white dark:bg-surface-800 rounded-2xl shadow-2xl border border-surface-200 dark:border-surface-700 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-200 dark:border-surface-700 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-200">
                {t('workspace.aiLeaderPrompt')} · {t('workspace.systemPrompt')}
              </h2>
              <p className="text-[11px] text-gray-400 dark:text-gray-500">
                {leaderAgent?.name || t('workspace.aiLeaderConfig')} · {leaderAgent?.avatar || '👑'}
                {isDirty && <span className="ml-2 text-amber-500">● {t('workspace.unsaved')}</span>}
                {!isDirty && !isDefaultPrompt && <span className="ml-2 text-teal-500">{t('workspace.customized')}</span>}
                {!isDirty && isDefaultPrompt && <span className="ml-2 text-gray-400">{t('workspace.defaultPrompt')}</span>}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={t('workspace.resetToDefaultPrompt')}
            >
              <RotateCcw size={13} />
              <span>{t('workspace.resetDefault')}</span>
            </button>
            {/* 关闭按钮 */}
            <button
              onClick={onClose}
          ...
          <div className="px-6 py-3 bg-orange-50 dark:bg-orange-900/20 border-b border-orange-200 dark:border-orange-800 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-orange-700 dark:text-orange-300">
                {t('workspace.confirmResetPrompt')}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button
        ...
        <div className="flex-1 flex flex-col min-h-0 p-4">
          <div className="flex items-center gap-2 mb-2 flex-shrink-0">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">{t('workspace.systemPrompt')}</span>
            <span className="text-[10px] text-gray-400 dark:text-gray-500 ml-auto">
              {t('workspace.promptCharsCount', { count: editingPrompt.length })}
            </span>
          </div>
          <textarea
            ref={textareaRef}
            ...
            placeholder={t('workspace.leaderSystemPromptPlaceholder')}
            spellCheck={false}
          />
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-3 border-t border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 flex-shrink-0">
          <p className="text-[10px] text-gray-400 dark:text-gray-500">
            {t('workspace.promptTakesEffectNextConversation')}
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
