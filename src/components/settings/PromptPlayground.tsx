...
export function PromptPlayground({ prompt, onBack }: PromptPlaygroundProps) {
  ...
  const handleRun = useCallback(async () => {
    if (!selectedProviderId || !selectedModelId) {
      setError(t('prompt.selectProviderAndModel'))
      return
    }
    ...
    try {
      ...
      if (!config) {
        setError(t('prompt.cannotResolveConfig'))
        setIsRunning(false)
        return
      }
      ...
      setError(err instanceof Error ? err.message : t('prompt.executionFailed'))
      setIsRunning(false)
    }
  }, [selectedProviderId, selectedModelId, resolveConfig, preview.content, t])
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="w-80 flex-shrink-0 border-r border-surface-200/80 dark:border-surface-700/60 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              <option value="">{t('prompt.selectProvider')}</option>
              {providers.map((p) => (
          ...
          {prompt.variables.length > 0 && (
            <div className="space-y-2">
              <label className="block text-xs font-medium text-surface-700 dark:text-surface-300 flex items-center gap-1.5">
                <Variable size={13} /> {t('prompt.variableValues')}
              </label>
            ...
            <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-800/30 rounded-lg p-2.5">
              <p className="text-xs text-amber-700 dark:text-amber-300">
                {validation.missing.length > 0 && (
                  <span>{t('prompt.missingRequiredVars', { vars: validation.missing.join(', ') })}</span>
                )}
                {validation.invalid.length > 0 && (
                  <span>{t('prompt.typeMismatch', { vars: validation.invalid.join(', ') })}</span>
                )}
              </p>
            </div>
          )}
            ...
            {error ? (
              ...
              <div className="flex items-center justify-center h-full text-muted">
                <div className="text-center">
                  ...
                  <p className="text-sm">{t('prompt.selectModelAndRun')}</p>
                  <p className="text-xs mt-1">{t('prompt.aiWillGenerate')}</p>
                </div>
              </div>
            )}
          </div>
    ...
    case 'select':
      return (
        <div>
          ...
          >
            <option value="">{variable.placeholder || t('prompt.pleaseSelect')}</option>
      ...
      return (
        <div>
          ...
          <input
            ...
            value={strValue}
            onChange={(e) => onChange(e.target.value)}
            placeholder={variable.placeholder || t('prompt.inputLabel', { label: variable.label || variable.name })}
