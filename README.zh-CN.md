...
| 🧰 **工具** | 内置联网/计算/知识库/数学/记忆/需求/网站分析/工作区工具；MCP；自定义工具；工具组权限与自动审批；**跨会话记忆**（`remember`/`recall`/`forget`/`list_memories`，支持按对话暂停注入与高级阻断开关） |
...
- **存储架构**  
  ...
  - **消息正文**：IndexedDB 库 `ConversationData`，按条存储
  - 惰性加载：内存中优先保留活跃对话消息  
